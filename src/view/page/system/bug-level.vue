<template>
    <div class="bug-box">
        <h4 class="bug-title">bug严重级别</h4>
        <div class="bug-btn">
            <el-button type="primary"  class="el-icon-plus" @click="addImportant">添加严重级别</el-button>
    ...
    export default {
        ...
        methods:{
            getData(){
                ...
                ajax.levelGet().then(res=>{
                    if(res.code==0){
                        this.tableData=res.levels
                        setTimeout(() => {
                            loading.close();
                        }, 1000);
                    }else{
            ...
            deleteRow(index,item){
                this.$confirm('此操作将删除, 是否继续?', '提示', {
                    ...
                    ajax.levelDel({id:item.id}).then(res=>{
                        if(res.code==0){
                            this.$message({
            ...
            onSubmit(){
                if(this.number==0){
                    ajax.levelAdd(this.form).then(res=>{
                        if(res.code==0){
                            this.$message({
                    ...
                    ajax.levelUpdate(this.form).then(res=>{
                        if(res.code==0){
                            this.$message({
                                message: res.msg || '添加成功',
                                type: 'success'
